...
function generateMatchEvents(
  ...
  if (userScore === rivalScore) {
    ...
    if (luck < userGoalProb) {
      ...
      events.push({ minute: 93, type: 'goal_rival', text: `💥 ¡El rival empata en tiempo añadido! El ${rivalTeam.pais} pasa a penales...` });
      // penalty logic
...
/* ─── COMPONENT ─────────────────────────────────────────── */
export default function ArmaEquipo() {
  ...
  /* helpers */
  const pickRandomTeam = useCallback((exclude: Set<number>) => {
  ...
  const startDraft = (expert: boolean) => {
    ...
    const team = pickRandomTeam(new Set());
    setCurrentTeam(team);
  ...
  const handleSlotClick = (slot: SlotInfo) => {
    ...
    if (slots[slot.key]) return; // already filled
    ...
    if (filledCount === 11) {
      ...
      const rival = pickRandomTeam(userIds);
      ...
      const userScore = events.filter(e => e.type === 'goal_user').length;
      const rivalScore = events.filter(e => e.type === 'goal_rival').length;
      const userWon = userScore > rivalScore || (events[events.length - 1]?.type === 'goal_user');
    ...
    if (nextRound < 11) {
      ...
      const nextTeam = pickRandomTeam(newUsed);
  ...
  const advanceTournament = () => {
    ...
    const rival = pickRandomTeam(usedRivalIds);
      ...
      <div className="min-h-screen flex flex-col items-center justify-center px-4 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-xl">
          <button
            ...
            className="group relative overflow-hidden rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-8 text-left hover:bg-emerald-500/20 transition-all duration-300 hover:scale-105 hover:shadow-xl hover:shadow-emerald-500/20"
          >
          ...
          <button
            ...
            className="group relative overflow-hidden rounded-2xl border border-purple-500/30 bg-purple-500/10 p-8 text-left hover:bg-purple-500/20 transition-all duration-300 hover:scale-105 hover:shadow-xl hover:shadow-purple-500/20"
          >
        ...
        <div className="mt-8 flex items-center gap-3 text-slate-500 text-sm">
          ...
          <span>•</span>
          <span>{DATA_MUNDIALES.length} equipos</span>
        </div>
      </div>
    );
  }
      ...
      <div className="flex flex-col lg:flex-row gap-6 min-h-screen px-2 py-6">
        ...
        <div className="lg:w-1/2 flex flex-col">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-xl font-bold text-white">Tu Equipo</h2>
              <p className="text-slate-400 text-sm">Ronda {round + 1}/11 · {filledSlots}/11 jugadores</p>
            </div>
            <div className="text-right">
              <div className="text-emerald-400 font-bold text-xl">{teamAvg > 0 ? teamAvg : '--'}</div>
          ...
          <div
            ...
            style={{
              background: 'repeating-linear-gradient(180deg, #166534 0px, #166534 40px, #15803d 40px, #15803d 80px)',
          ...
          >
            ...
            <div className="absolute inset-0 pointer-events-none">
              ...
              <div className="absolute bottom-0 left-1/4 right-1/4 h-16 border-t border-l border-r border-white/20" />
              <div className="absolute top-0 left-1/4 right-1/4 h-16 border-b border-l border-r border-white/20" />
            </div>

            {/* Slots grid */}
            <div className="absolute inset-4 grid grid-rows-6 gap-1">
              {sortedRows.map(rowIdx => {
                const rowSlots = slotsByRow[rowIdx];
                return (
                  <div key={rowIdx} className="flex items-center justify-around">
                    {rowIdx === 5 ? (
                      // Goalkeeper row — single centered
          ...
          {currentTeam && (
            <>
              <div className="flex items-center gap-3 mb-4">
                <span className="text-4xl">{currentTeam.bandera}</span>
                <div>
                  <h3 className="text-lg font-bold text-white">
                    {currentTeam.pais} {currentTeam.anio}
                  </h3>
                  <p className="text-slate-400 text-sm">Selecciona un jugador y luego una posición compatible</p>
                </div>
              </div>
                  ...
                  return (
                    <button
                      ...
                      className={`
                        ...
                        ${isSelected
                          ? 'border-emerald-400 bg-emerald-400/15 scale-105 shadow-lg shadow-emerald-500/30'
                          : 'border-slate-700 bg-slate-800/60 hover:border-slate-500 hover:bg-slate-700/60'
                        }
                      `}
                    >
                      <div className="flex items-start justify-between mb-2">
                        <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${getPosColor(jugador.posicion)}`}>
                          {jugador.posicion}
                        </span>
                        <span className={`text-lg font-black ${expertMode ? 'text-slate-600' : getRatingColor(jugador.rating)}`}>
                          {expertMode ? '?' : jugador.rating}
                        </span>
                      </div>
                      <p className="text-white font-semibold text-sm leading-tight">{jugador.nombre}</p>
                      {isSelected && (
              ...
              {selectedPlayer && (
                <div className="mt-4 p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 text-sm">
                  <strong>{selectedPlayer.nombre}</strong> seleccionado. Haz clic en una posición compatible en el campo.
                  <span className="ml-2 text-slate-400">({selectedPlayer.posicion})</span>
                </div>
              )}
            </>
          )}
    ...
    return (
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          <span className={`text-sm font-bold uppercase tracking-widest ${phaseColor[phase] ?? 'text-white'}`}>
            {phase} de Final
          </span>
          <h2 className="text-3xl font-black text-white mt-1">
            Tu Equipo <span className="text-slate-400">vs</span> {rival.bandera} {rival.pais} {rival.anio}
          </h2>
          <div className="flex items-center justify-center gap-6 mt-4">
            <div className="text-center">
              <div className="text-slate-400 text-xs mb-1">Media tu equipo</div>
              <div className="text-3xl font-black text-emerald-400">{userAvg}</div>
            </div>
            <div className="text-4xl font-black text-white">{userScore} – {rivalScore}</div>
            <div className="text-center">
              <div className="text-slate-400 text-xs mb-1">Media {rival.pais}</div>
              <div className={`text-3xl font-black ${getRatingColor(rivalAvg)}`}>{rivalAvg}</div>
            </div>
          </div>
          <div className={`mt-3 text-xl font-bold ${userWon ? 'text-emerald-400' : 'text-red-400'}`}>
            {userWon ? '✅ ¡Avanzas de ronda!' : '❌ Eliminado'}
          </div>
        </div>
        ...
        >
          {showEvents ? '▲ Ocultar Eventos' : '▼ Ver Relato del Partido'}
        </button>
        ...
        {showEvents && (
          <div className="space-y-2 mb-6 max-h-72 overflow-y-auto pr-1">
            {events.map((ev, i) => (
              ...
              >
                <span className="font-bold text-xs shrink-0 pt-0.5">
                  {ev.minute}&apos;
                </span>
                <span>{ev.text}</span>
              </div>
          ...
          {PHASES.map((p, i) => (
            <div key={p} className="flex items-center gap-2">
              <div className={`px-3 py-1 rounded-full text-xs font-bold ${
                ...
                i === tournament.phaseIndex ? 'bg-white/15 text-white border border-white/30' :
        ...
        >
          {userWon
            ? tournament.phaseIndex < PHASES.length - 1 ? '→ Siguiente Fase' : '🏆 Ver Resultado Final'
            : 'Ver resultado de eliminación'}
        </button>
      </div>
    );
  }
  ...
  if (gamePhase === 'CHAMPION') {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-4 py-12 text-center">
        ...
        <h1 className="text-5xl md:text-7xl font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 via-amber-400 to-yellow-600 mb-4 animate-pulse">
          ¡CAMPEÓN DEL MUNDO!
        </h1>
        <p className="text-slate-300 text-xl max-w-lg mb-3">
          Tu equipo drafteado conquistó el torneo mundial. ¡Eres un genio del fútbol!
        </p>
        <div className="flex flex-wrap gap-3 justify-center mb-8 mt-2">
          {PHASES.map(p => (
            <div key={p} className="px-4 py-2 rounded-full bg-yellow-400/15 border border-yellow-400/40 text-yellow-300 text-sm font-bold">
              ✅ {p}
            </div>
          ))}
        </div>
        {/* Team summary */}
        <div className="grid grid-cols-3 gap-3 mb-8 max-w-lg w-full">
          {FIELD_SLOTS.map(slot => {
            const j = slots[slot.key];
            if (!j) return null;
            return (
              <div key={slot.key} className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-3">
                <div className="text-xs text-yellow-600 font-bold mb-1">{slot.label}</div>
                <div className="text-white text-sm font-semibold leading-tight">{j.nombre}</div>
                <div className="text-yellow-400 font-black text-lg">{j.rating}</div>
              </div>
            );
          })}
        </div>
        <button
          onClick={resetGame}
          className="px-10 py-4 bg-gradient-to-r from-yellow-500 to-amber-500 rounded-2xl text-black font-black text-xl hover:scale-105 transition-transform shadow-2xl shadow-yellow-500/40"
        >
          Volver al Hub
        </button>
      </div>
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-4 py-12 text-center">
        ...
        <p className="text-slate-400 text-lg max-w-md mb-8">
          Tu equipo llegó hasta {eliminatedPhase} de Final. ¡Buen intento! La próxima será.
        </p>
        <div className="grid grid-cols-3 gap-2 mb-8 max-w-md w-full opacity-60">
          {FIELD_SLOTS.map(slot => {
            const j = slots[slot.key];
            if (!j) return null;
            return (
              <div key={slot.key} className="bg-red-500/10 border border-red-500/20 rounded-xl p-2">
                <div className="text-xs text-red-600 font-bold">{slot.label}</div>
                <div className="text-white text-xs">{j.nombre}</div>
              </div>
            );
          })}
        </div>
        <button
          onClick={resetGame}
          className="px-10 py-4 bg-gradient-to-r from-slate-700 to-slate-600 border border-slate-500 rounded-2xl text-white font-bold text-lg hover:scale-105 transition-transform"
        >
          Intentar de nuevo
        </button>
      </div>
...
function SlotButton({
  ...
  return (
    <button
      onClick={onClick}
      className={`
        relative flex flex-col items-center justify-center rounded-xl p-2 transition-all duration-200 text-center w-20 h-20 text-xs
        ${isFilled
          ? 'bg-emerald-600/80 border-2 border-emerald-400 shadow-lg shadow-emerald-500/30'
          : isCompatible
            ? 'bg-yellow-400/20 border-2 border-yellow-400 animate-pulse cursor-pointer'
            : 'bg-black/30 border-2 border-white/20 hover:border-white/40'
        }
      `}
    >
      {isFilled ? (
        <>
          <span className={`font-black text-sm ${expertMode ? 'text-slate-300' : getRatingColor(jugador!.rating)}`}>
            {expertMode ? '?' : jugador!.rating}
          </span>
        ...
        <>
          <span className="text-white/50 text-[10px] font-bold">{slot.label}</span>
          {isCompatible && <span className="text-yellow-400 text-lg">+</span>}
        </>
      )}
    </button>
  );
